...
public class GameTestTerminalCraftingOptionMachines {
    ...
    private static CompoundTag serializeCraftingOption(GameTestHelper helper,
                                                       TerminalStorageTabIngredientCraftingHandlerCraftingNetwork handler,
                                                       TerminalCraftingOptionRecipeDefinition<?, ?> craftingOption) {
        try (ProblemReporter.ScopedCollector scopedCollector = new ProblemReporter.ScopedCollector(LOGGER)) {
            TagValueOutput valueOutput = TagValueOutput.createWithContext(scopedCollector, helper.getLevel().registryAccess());
            handler.serializeCraftingOption(valueOutput, craftingOption);
            return valueOutput.buildResult();
        }
    }

    private static TerminalCraftingOptionRecipeDefinition<?, ?> deserializeCraftingOption(GameTestHelper helper,
                                                                                       TerminalStorageTabIngredientCraftingHandlerCraftingNetwork handler,
                                                                                       CompoundTag tag) {
        try (ProblemReporter.ScopedCollector scopedCollector = new ProblemReporter.ScopedCollector(LOGGER)) {
            ValueInput valueInput = TagValueInput.create(scopedCollector, helper.getLevel().registryAccess(), tag);
            return handler.deserializeCraftingOption(valueInput, IngredientComponent.ITEMSTACK);
        }
    }
    ...
    public void testCraftingMachinesSerialization(GameTestHelper helper) {
        ...
        CompoundTag tag = serializeCraftingOption(helper, handler, craftingOption);
        TerminalCraftingOptionRecipeDefinition<?, ?> deserialized = deserializeCraftingOption(helper, handler, tag);
    ...
    protected void testCraftingMachinesInNetwork(GameTestHelper helper, boolean attuned, Block[] crafters,
                                                 ...
                                                 RecipeType<?> recipeType, Identifier recipeName) {
    ...
    @GameTest(template = Reference.MOD_ID + ":empty10", timeoutTicks = TIMEOUT)
    public void testCraftingMachinesInNetworkCraftingTable(GameTestHelper helper) {
        testCraftingMachinesInNetwork(helper, false, new Block[]{Blocks.CRAFTING_TABLE}, 1, Items.CRAFTING_TABLE,
                RecipeType.CRAFTING, Identifier.fromNamespaceAndPath("minecraft", "chest"));
    }
    ...
    public void testCraftingMachinesInNetworkFurnace(GameTestHelper helper) {
        testCraftingMachinesInNetwork(helper, false, new Block[]{Blocks.FURNACE}, 1, Items.FURNACE,
                RecipeType.SMELTING, Identifier.fromNamespaceAndPath("minecraft", "iron_ingot_from_smelting_raw_iron"));
    }
