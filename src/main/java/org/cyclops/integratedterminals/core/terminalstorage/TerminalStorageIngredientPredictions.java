...
/**
 * Client-side predictions of storage changes that the player caused,
 ...
 * Before that, the server reports what its click really moved, which cuts a prediction
 * that expected too much down to size within one round trip.
 * Whatever is still unconfirmed after that expires, so a wrong prediction always corrects itself.
 *
...
public class TerminalStorageIngredientPredictions<T, M> {
     ...
     * @param clickId The click that caused this, so that the server can correct it.
     ...
     */
    public void add(int clickId, int channel, T instance, boolean addition) {
        if (!this.ingredientComponent.getMatcher().isEmpty(instance)) {
            this.predictions.add(new Prediction<>(clickId, channel, instance, addition,
                    System.currentTimeMillis() + EXPIRY_TIME_MS));
        }
    }

    /**
     * Correct the prediction of the given click to what the server reports that it really moved.
     * @param clickId The click that was reported on.
     * @param movedQuantity The quantity that the click moved.
     * @return If a prediction was corrected.
     */
    public boolean reconcile(int clickId, long movedQuantity) {
        ...
        while (it.hasNext()) {
            ...
            if (prediction.getClickId() == clickId) {
                long predicted = matcher.getQuantity(prediction.getInstance());
                if (movedQuantity >= predicted) {
                    // We did not predict too much, so there is nothing to correct
                    return false;
                }
                if (movedQuantity <= 0) {
                    it.remove();
                } else {
                    prediction.setInstance(matcher.withQuantity(prediction.getInstance(), movedQuantity));
                }
                return true;
            }
        }
    ...
    public static class Prediction<T> {
        ...
        public Prediction(int clickId, int channel, T instance, boolean addition, long expiryTime) {
