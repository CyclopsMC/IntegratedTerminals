...
public class TerminalStorageTabEnderChestCommon implements ITerminalStorageTabCommon {
    ...
    public TerminalStorageTabEnderChestCommon(ContainerTerminalStorageBase<?> container, Identifier name) {
        ...
        for (int row = 0; row < ROWS; row++) {
            for (int column = 0; column < COLUMNS; column++) {
                ...
                slots.add(Pair.of(slot, factors -> Pair.of(
                        factors.offsetX() + ITerminalStorageTabClient.DEFAULT_SLOT_OFFSET_X + finalColumn * IModHelpers.get().getGuiHelpers().getSlotSize(),
                        factors.offsetY() + ITerminalStorageTabClient.DEFAULT_SLOT_OFFSET_Y + finalRow * IModHelpers.get().getGuiHelpers().getSlotSize()
                )));
            }
        }
